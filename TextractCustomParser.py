...
class TextractObject:
	...
	FORMS_ISSUE_DATE_KEYWORDS = ['invoice date', 'date', 'date:', 'inv date:', 'inv date', 'invoice date:', 'statement date', 'i invoice date', 'entered date', 'expiration date']
	FORMS_DUE_DATE_KEYWORDS = ['due date']
	FORMS_PAYEE_REFERENCE_KEYWORDS = ['invoice', 'invoice #', 'invoice number', 'invoice no', 'invoice no.', 'Invoice', 'Invoice #', 'Invoice Number', 'Invoice No', 'Invoice No.', 'invoice:', 'Invoice:', 'INVOICE:', 'i nvoi ce #', 'order number']
	FORMS_QUOTE_NUMBER_KEYWORDS = ['estimate #', 'order number', 'quote no.', 'quote #', 'quote number']
	FORMS_TAX_KEYWORDS = ['tax', 'taxes', 'sales tax', 'total tax', 'sales tax:']
	FORMS_CO_AMOUNT_KEYWORDS = ['change order total', 'total agreement price', 'value this change order', 'approved amount', 'amount', 'total cost for change order:', 'total:']
	FORMS_AMT_KEYWORDS = ['sale', 'sale total', 'amt paid', 'credit', 'total', 'total:', 'Charge', 'charge', 'Amount', 'amount', 'Amt', 'amt', 'invoice amt.', 'inv total', 'invoice amount', 'invoice total', 'inv amount', 'net amt w/o tax', 'amount due', 'total for this invoice']
	TABLE_LINE_NUM_KEYWORDS = ['Ln #', 'Line #', 'ln #', 'line #']
	TABLE_ITEM_KEYWORDS = ['ITEM', 'Item', 'Item #', 'Product And Description', 'item', 'item #', 'product and description']
	TABLE_QTY_KEYWORDS = ['Inv Qty', 'QTY SHIPPED', 'Invoiced', 'Quantity Shipped', 'Qty', 'QTY', 'qty', 'qty.', 'quantity shipped', 'Qty Shipped', 'qty shipped', 'inv qty', 'invoiced', 'shipped', 'ship']
	TABLE_QTY_ORDERED_KEYWORDS = ['Quantity Ordered', 'quantity ordered', 'qty ordered', 'qty. ordered', 'order', 'qty ord']
	TABLE_UNIT_PRICE_KEYWORDS = ['Unit Price', 'NET PRICE', 'Rate', 'rate', 'RATE', 'price', 'unit price', 'net price']
	TABLE_AMT_KEYWORDS = ['EXT PRICE', 'Amount (Net)', 'Amount', 'Ext Amount', 'Total', 'TOTAL', 'total', 'Extension', 'extension', 'ext price', 'amount (net)', 'amount', 'ext amount', 'item subtotal before tax', 'extended amount']
	TABLE_VPN_KEYWORDS = ['MPN', 'mpn', 'ITEMS', 'items', 'ITEM', 'Item', 'item', 'product and description']
	TABLE_VPD_KEYWORDS = ['DESCRIPTION', 'Description', 'description']
	TABLE_INVOICE_KEYWORDS = ['Invoice', 'invoice', 'transaction', 'Transaction', 'Invoice No.', 'invoice no.', 'Invoice No', 'Invoice Number', 'invoice no', 'invoice number', 'invoice #', 'Invoice #', 'Activity', 'activity', 'inv # job', 'description', 'invoice/c', 'document number']
	TABLE_INVOICE_AMT_KEYWORDS = ['Charge', 'charge', 'Amount', 'amount', 'Amt', 'amt', 'invoice amt.', 'inv total', 'invoice amount', 'invoice total', 'inv amount', 'net amt w/o tax', 'amount due', 'charges']
	TABLE_INVOICE_CREDIT_KEYWORDS = ['Credit', 'credit', 'credit applied', 'invoice credit', 'inv credit', 'inv. credit']
	TABLE_INVOICE_BALANCE_KEYWORDS = ['Balance', 'balance', 'inv balance', 'invoice balance', 'inv. balance', 'open amount', 'balance due']
	...
	def __init__(self, response, output_path = ''):
	...
	def parseJSON(self):
		if self.document != None:
			...
			for page in doc.pages:
				...
				if self.DOC_MAP[page_type] in PO_DOCTYPES and self.po_value != '':
					page_key = self.po_value
				
				if page_key not in self.pages_json.keys():
					self.pages_json[page_key] = {
						"page_type" : self.DOC_MAP[page_type],
						"tables" : {},
						"forms" : {}
					}
				else:
					if self.pages_json[page_key]["page_type"] != self.DOC_MAP[page_type]:
						page_key += '-' + self.DOC_MAP[page_type]
