...
def sf_authenticate():
	...
	print('SF_URL >>> ' + SF_URL)
	print('SF_CLIENT_ID >>> ' + SF_CLIENT_ID)
	print('SF_CLIENT_SECRET >>> ' + SF_CLIENT_SECRET)
	print('SF_PASSWORD >>> ' + SF_PASSWORD)
...
def start(textMessage):
	...
	# #  If processed through the document upload, there's no parent Id
	# if objectAPIName != 'Document_Manager__c':
	# 	parent_docManager = create_record('Document_Manager__c', {
	# 		"Document_Type__c" : 'Source Document',
	# 		"Status__c" : "Complete",
	# 		"Analysis_Status__c" : "Complete",
	# 		"Subject__c" : pKey + " Source Document"
	# 		# "Record_Id__c" : s3_data["Record_Id__c"]
	# 	}, instance_url=instance_url, access_token=access_token)
		
	# 	if parent_docManager["success"] == True:
	# 		parentId = parent_docManager["id"]
			
	#  Fix for unidentified
	docCategory = pageType
	docType = 'Processed Document'
	
	# if pageType != 'Unidentified':
	# 	docCategory = pageType
	# else:
